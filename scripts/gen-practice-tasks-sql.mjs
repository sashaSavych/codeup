...
const tasks = [
  {
    ...
    title: 'Перший рядок',
    description:
      'Оголоси **константу** `hello` з рядком `"CodeUp"` (лапки як у прикладі). У теорії цієї теми — розділ «Константа й рядок». Нічого не виводь у консоль — перевірка читає значення змінної.\n\n## Критерії успіху\n- Є константа `hello`\n- Значення — рядок `"CodeUp"`',
  ...
  },
  {
    ...
    description:
      'Напиши функцію **greet(name)**, яка **повертає** (`return`) рядок виду `Привіт, Імʼя!` (з пробілом після коми). Можна склеїти частини через `+`. У теорії — розділ «Найпростіша функція та `return`».\n\n## Критерії успіху\n- Є функція `greet` з одним параметром\n- `greet("Оля")` повертає рядок `Привіт, Оля!`',
